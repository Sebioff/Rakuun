...
class Rakuun_Intern_GUI_Panel_Shoutbox_Meta extends Rakuun_Intern_GUI_Panel_Shoutbox {
	public function __construct($name, $title = '') {
		...
		$this->config->setShoutContainer(Rakuun_DB_Containers::getShoutboxMetasContainer()->getFilteredContainer($options));
		$shout = new DB_Record();
		$shout->user = $user;
		$shout->meta = $user->alliance->meta;
		...
		$this->config->setDeleteQuery('
			...
			WHERE ID <= (
				...
				FROM(
					...
					WHERE meta = '.$user->alliance->meta->getPK().'
					ORDER BY date DESC
					LIMIT 1
					OFFSET 100
				) as temp
