...
class Rakuun_Intern_GUI_Panel_Statistics_User_DestroyedBuildings extends GUI_Panel {
	public function init() {
		parent::init();
		$this->setTemplate(dirname(__FILE__).'/destroyedbuildings.tpl');
		
		$this->addPanel($table = new GUI_Panel_Table('statistics'));
		
		$table->addHeader(array('Gebäude', 'Vernichtete Stufen'));
		
		$options = array();
		$options['properties'] = 'SUM(delta_level) as lost_levels, building';
		$options['conditions'][] = array('executing_user = ?', Rakuun_User_Manager::getCurrentUser());
