...
/**
 * Displays all ressource transfers of a given user
 */
	...
	public function init() {
		...
		if ($this->user) {
			...
			foreach (Rakuun_DB_Containers::getLogUserRessourcetransferContainer()->selectByUser($this->user, $options) as $ressourcetransfer) {
				...
				$log->addLine(array($sender, Rakuun_Intern_Log::getActionDescription($ressourcetransfer->action), $date, $ressourcetransfer->iron, $ressourcetransfer->beryllium, $ressourcetransfer->energy, $ressourcetransfer->people, $ressourcetransfer->ip, $ressourcetransfer->hostname, $ressourcetransfer->browser));
			}
		}
	}
}
